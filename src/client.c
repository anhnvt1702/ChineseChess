...
void update_ui_from_background(struct Response *response)
{
    // Tạo dialog
    GtkWidget *dialog = gtk_dialog_new_with_buttons("Thông báo", GTK_WINDOW(chessgame_window), GTK_DIALOG_MODAL, "Chấp nhận", GTK_RESPONSE_ACCEPT, "Từ chối", GTK_RESPONSE_REJECT, NULL);

    // Lấy content area của dialog để thêm nội dung
    GtkWidget *content_area = gtk_dialog_get_content_area(GTK_DIALOG(dialog));

    // Tạo label với nội dung thông báo
    gchar *message = g_strdup_printf("%s đã mời bạn tham gia trận đấu. Chấp nhận không?", response->data.resinvite.resInvitefrom);
    ...
    case GTK_RESPONSE_ACCEPT:
        ...
        create_window_main(response->data.resinvite.port, response->data.resinvite.resInviteto);
        g_print("Đã chấp nhận lời mời.\n");
        break;
    case GTK_RESPONSE_REJECT:
        // Thực hiện hành động khi từ chối
        g_print("Đã từ chối lời mời.\n");
        break;
    }
...
void *background_task(void *data)
{
    ...
    while (1)
    {
        int recv_size = recv(client_socket, received_response, sizeof(struct Response), 0);
        ...
        else if (received_response->messageType == INVITE_REQUEST)
        {
            ...
            {
                    g_idle_add((GSourceFunc)update_ui_from_background, received_response);
            }
        }
    }
