                ...
                case INVITE_REQUEST:
                    ...
                    for (int j = 0; j < max_clients; j++)
                    {
                        ...
                        if (client_sd > 0 && strcmp(userInSocket[j], received_request.data.invite.inviteto) == 0 && is_socket_logged[j] == 1)
                        {
                            ...
                            strcpy(response.data.resinvite.resInvitefrom, received_request.data.invite.invitefrom);
                            strcpy(response.data.resinvite.resInviteto, received_request.data.invite.inviteto);
                            ...
                            //printf("%ld\n", send(client_sd, &hihi, sizeof(response), 0)); // Pass the address of the structure
